...
def x2nodes(data):
    ...
    for i in range(batch_size):
        list_x.append(torch.mean(data.x[data.batch == i, 0:5], dim=0))
    return torch.stack(list_x, dim=0)

def edges2association(data):
    edges_real = torch.zeros(32, 10, 10, 4).to(device)
    for i in range(data.edge_index.shape[1]):
        ...
        edges_real[data.batch[node1], min(idx1, 9), min(idx2, 9), ] = data.edge_attr[i]
...
def train(train_loader, epoch_i):
    ...
    for data in train_loader:
        ...
        # 2. Train VAE model
        x, x_mu, x_logvar = encoder(data.x[:, 0:5], data.edge_index)
        edges_logits, nodes_logits = decoder(x)
        (edges_hat, nodes_hat) = postprocess((edges_logits, nodes_logits))

        nodes_real = x2nodes(data)
        n_loss = torch.nn.CrossEntropyLoss(reduction='mean')(torch.mean(nodes_hat, dim=1).view(-1, m_dim)[0:len(data), ], nodes_real[0:len(data), ])
...
torch.save(decoder.state_dict(), '../model/MolVAE_QM9_decoder.pth')
torch.save(encoder.state_dict(), '../model/MolVAE_QM9_encoder.pth')
torch.save(V.state_dict(), '../model/MolVAE_QM9_V.pth')
